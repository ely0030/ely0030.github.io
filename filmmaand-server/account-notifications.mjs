...
export function commitActivity(c,before,at,{scheduled=false}={}){
 ...
 for(const [planId,row] of Object.entries(c.state.plans||{})){
  ...
  function emit(type,key,text,{actorId=null,recipient=null,recipients=null,excludeRecipients=null,href='/filmmaand/films/',count=1,group=true,subjectIds=[]}={}){
   const subjects=subjectIds.slice(0,3).map(id=>p.options.find(o=>o.id===id)).filter(Boolean).map(o=>({id:o.id,title:clean(o.title),poster:o.image?.url||o.movie?.poster||o.movies?.[0]?.poster||null}));
   ...
   for(const person of people){const personId='p_'+person.id;if(recipient&&recipient!==personId||recipients&&!recipients.has(personId)||excludeRecipients?.has(personId)||actorId===personId)continue;const account=n.accounts[person.id]||={items:[],importantActivityEmail:true};retain(account,at);const groupKey=hash([planId,type,key]),old=account.items.find(i=>i.groupKey===groupKey&&(type==='suggestion-liked'||Date.parse(at)-Date.parse(i.updatedAt)<10*60*1000));
    if(group&&old){old.text=clean(text);old.href=href;old.actor=actor;old.subjects=subjects;old.updatedAt=new Date(Math.max(Date.parse(at),Date.parse(old.updatedAt)+1)).toISOString();old.readAt=null;old.count=type==='suggestion-liked'?old.count+count:count;if(type==='suggestion-liked')old.text=clean(text.replace('een hartje.',old.count+' keer een hartje.'));}
    else account.items.push({id:hash([groupKey,at,p.version]),groupKey,type,text:clean(text),href,actor,subjects,count,createdAt:at,updatedAt:at,readAt:null});retain(account,at);
   }
  }
  const recommendedRecipients=new Set();
  for(const o of after.suggestions){if(!prior.suggestions.some(x=>x.id===o.id)&&o.owner)emit('film-suggested',o.id,title(o.id)+' is voorgesteld.',{subjectIds:[o.id],actorId:o.owner,group:false,href:'/filmmaand/films/?film='+encodeURIComponent(o.id)});
   const likers=Object.entries(after.likes).filter(([a,ids])=>a!==o.owner&&ids.includes(o.id)&&!(prior.likes[a]||[]).includes(o.id));if(o.owner&&likers.length)emit('suggestion-liked',o.id,'Je voorstel '+title(o.id)+' kreeg een hartje.',{subjectIds:[o.id],recipient:o.owner,actorId:likers.at(-1)[0],count:likers.length,href:'/filmmaand/films/?film='+encodeURIComponent(o.id)});
  ...
  const entered=after.entries.filter(id=>!prior.entries.includes(id));if(entered.length)emit('leaderboard-entry',roundId,titles(entered)+' staat nu in de top voor de volgende ronde.',{subjectIds:entered,excludeRecipients:recommendedRecipients,href:entered.length===1?'/filmmaand/films/?film='+encodeURIComponent(entered[0]):'/filmmaand/films/'});
  if(!same(after.numberOne,prior.numberOne))emit('next-leader',roundId,after.numberOne.length>1?'Gedeeld bovenaan voor de volgende ronde: '+titles(after.numberOne):after.numberOne.length?titles(after.numberOne)+' staat bovenaan voor de volgende ronde.':'Er is nog geen koploper voor de volgende ronde.',{subjectIds:after.numberOne,excludeRecipients:recommendedRecipients});
  if(after.round.id===prior.round.id&&after.round.status==='open'&&!same(after.leaders,prior.leaders))emit('vote-leader',roundId,after.leaders.length>1?'Gelijke stand in de stemming: '+titles(after.leaders):after.leaders.length?titles(after.leaders)+' gaat aan kop in de stemming.':'De stemming heeft nog geen koploper.',{subjectIds:after.leaders,href:'/filmmaand/stemmen/'});
  let mailType,text;
  ...
  if(mailType){const href=after.round.result?.programmeId?'/filmmaand/programma/':'/filmmaand/stemmen/';emit(mailType,roundId,text,{href,subjectIds:mailType==='round-opened'?after.shortlist:after.round.result?[after.round.result.choice]:after.leaders});const id=hash([planId,roundId,mailType,after.round.status]);n.mailIntents[id]={id,type:mailType,planId,eventId:roundId,occurredAt:at,scheduledDate:p.roundSchedule?.date||null,title:text,href,programmeId:after.round.result?.programmeId||null};}
 }
