...
async function fixture(){
 ...
 const draw=(who,strokes,extra={})=>request(PLAN+'date-poll-doodle','PUT',{pollId,strokes,...extra},{'X-Filmmaand-Poll-Pass':pass[who],'Idempotency-Key':key()});
...
test('a pass holder adds and replaces their own doodle; everyone in the poll sees it with name and time',async()=>{
 ...
 assert.deepEqual(saved.body.strokes,[['k',[[10,10.1],[50,90],[90,10]]],['r',[[50,50]]]]);// rounded to 0.1
 assert.equal(saved.body.at,'2026-09-23T10:00:00.000Z');assert.match(saved.body.id,/^doodle-[0-9a-f]{16}$/);
 const daan=(await f.view('Daan')).body.doodles;
 assert.deepEqual(daan,[{id:saved.body.id,name:'Lotte',avatarId:12,at:saved.body.at,strokes:saved.body.strokes}]);
 ...
 const again=await f.draw('Lotte',[['r',[[1,2]]]]);assert.equal(again.body.id,saved.body.id);
 assert.equal(Object.keys(f.doodles()).length,1);assert.deepEqual((await f.view('Daan')).body.doodles.map(d=>[d.name,d.at,d.strokes]),[['Lotte','2026-09-23T10:05:00.000Z',[['r',[[1,2]]]]]]);
...
test('scope: a pass writes only its holder\'s own doodle and nothing else; no field selects whose doodle',async()=>{
 ...
 const ok=await f.draw('Lotte',STAR);assert.equal(ok.status,200);
 const after=f.store.data,strip=s=>{s=structuredClone(s);const d=s.plans['home-picker-lab'].data;
  for(const [a,v] of Object.entries(d.datePoll.doodles))if(v.id===ok.body.id)delete d.datePoll.doodles[a];
  d.receipts=Object.fromEntries(Object.entries(d.receipts).filter(([,v])=>v.result?.id!==ok.body.id));delete d.version;delete s.plans['home-picker-lab'].version;return s};
 ...
 const hide=await f.request(PLAN+'date-poll','POST',{action:'hide-doodle',pollId:f.pollId,doodleId:ok.body.id,hidden:true},{'X-Filmmaand-Poll-Pass':f.pass.Lotte,'Idempotency-Key':f.key()});
 assert.equal(hide.status,405);
 // A dead pass is the uniform 401 and writes nothing.
 const dead=await f.request(PLAN+'date-poll-doodle','PUT',{pollId:f.pollId,strokes:STAR},{'X-Filmmaand-Poll-Pass':'A'.repeat(43),'Idempotency-Key':f.key()});
...
test('hard size cap and shape: 4KB after rounding, 64 strokes, 2000 points, two inks, 0..100',async()=>{
 ...
 for(const bad of [[],'x',[['b',[[1,1]]]],[['k',[]]],[['k',[[101,1]]]],[['k',[[-1,1]]]],[['k',[[1,'2']]]],[['k',[[1,2,3]]]],[['k',[[NaN,1]]]],[['k',[[1,1]],'extra']]])
...
test('organiser kill switch: hide, sticky across a replacement, unhide; organiser view lists all',async()=>{
 const f=await fixture(),mine=(await f.draw('Lotte',STAR)).body;f.clock.now='2026-09-23T10:01:00.000Z';await f.draw('Daan',STAR);
 ...
 assert.deepEqual((await f.view('Daan')).body.doodles.map(d=>[d.name,d.strokes]),[['Lotte',[['k',[[5,5]]]]],['Daan',normaliseStrokes(STAR)]]);
...
test('never mail, never public: no notice or outbox entry, public plan GET has no doodles, closed poll refuses',async()=>{
 ...
 const viaSession=await f.request(PLAN+'date-poll-doodle','PUT',{pollId:f.pollId,strokes:STAR},{Cookie:cookie,'Idempotency-Key':f.key()});
