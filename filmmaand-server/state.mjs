...
const TABLES=['participants','login_codes','sessions','rate_limits','claims','receipts','password_credentials','email_ownership','session_security','auth_invites','poll_passes'];
// Tables added after launch: absent until first row, so deploying them never turns a read-only request into a state write.
const OPTIONAL_TABLES=new Set(['poll_passes']);
...
export function openState(input){
 ...
 return {state,authStore,plans,export(){for(const table of TABLES){const rows=authStore.db.prepare(`SELECT * FROM ${table} ORDER BY rowid`).all();if(OPTIONAL_TABLES.has(table)&&!rows.length&&!Object.hasOwn(state.auth,table))continue;state.auth[table]=rows}return state},close(){authStore.close()}};
}
