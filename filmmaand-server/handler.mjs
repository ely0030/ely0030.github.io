...
async function initialize(){
 ...
 const movieCatalogue=createMovieDiscovery(catalogue,{creditsPath:new URL('./data/movie-credits.sqlite',import.meta.url).pathname,artwork:JSON.parse(await readFile(new URL('./runtime/planning/movie-artwork.json',import.meta.url))),metadataToken:env.TMDB_READ_ACCESS_TOKEN||'',metadataApiKey:env.TMDB_API_KEY||'',ratingsApiKey:env.MDBLIST_API_KEY||''});
