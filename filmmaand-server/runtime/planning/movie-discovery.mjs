...
export function createMovieDiscovery(catalogue,{fetcher=fetch,timeout=1800,cachePath=null,metadataToken='',metadataApiKey='',ratingsApiKey='',artwork={},creditsPath=fileURLToPath(new URL('./.data/movie-credits.sqlite',import.meta.url))}={}){
 const ratings=createMovieRatings({cachePath,fetcher,apiKey:ratingsApiKey});
 ...
 const attach=movie=>{if(!movie)return null;const out={...movie,rtRatings:ratings.get(movie.id),...metadata.get(movie.id),...(extras.get(movie.id)||JSON.parse(artRead?.get(movie.id)?.data||'{}')),...(artwork[movie.id]||{}),directors:creditQuery?JSON.parse(creditQuery.get(movie.id)?.directors||'[]'):[]};if(!out.poster&&out.metadataPoster){out.poster=out.metadataPoster;out.posterFull=out.metadataPoster;}return out;};
 ...
 return{async search(query,limit=8,{waitForArtwork=true}={}){const movies=catalogue.search(query,limit);if(!waitForArtwork)return movies.map(attach);let next=0;await Promise.all(Array.from({length:Math.min(4,movies.length)},async()=>{while(next<movies.length){const m=movies[next++];if(!attach(m).poster)await enrich(m.id)}}));return movies.map(attach)},async details(id,{includeMetadata=true}={}){const movie=catalogue.get(id);if(!movie)return null;await Promise.all([!attach(movie).poster?enrich(id):null,includeMetadata?metadata.details(id):null,includeMetadata?ratings.details(id):null]);const out=attach(movie);if(!out.poster&&out.metadataPoster)out.poster=out.posterFull=out.metadataPoster;return out},get(id){return attach(catalogue.get(id))},close(){ratings.close();metadata.close();artDB?.close();credits?.close();catalogue.close()}};
}
