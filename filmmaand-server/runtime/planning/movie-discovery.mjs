...
export function createMovieDiscovery(catalogue,{fetcher=fetch,timeout=1800,cachePath=null,metadataToken='',metadataApiKey='',artwork={},creditsPath=fileURLToPath(new URL('./.data/movie-credits.sqlite',import.meta.url))}={}){
 ...
 const attach=movie=>{if(!movie)return null;const out={...movie,...metadata.get(movie.id),...(extras.get(movie.id)||JSON.parse(artRead?.get(movie.id)?.data||'{}')),...(artwork[movie.id]||{}),directors:creditQuery?JSON.parse(creditQuery.get(movie.id)?.directors||'[]'):[]};if(!out.poster&&out.metadataPoster){out.poster=out.metadataPoster;out.posterFull=out.metadataPoster;}return out;};
