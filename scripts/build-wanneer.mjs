...
// The curtain (Chris, 23 Sept: "make sure we don't accidentally load the page first before the movie if they haven't seen it; this is vital").
// Without it a first-time visitor saw the chat for the first read + the bundle download (and a tap in that window cancelled the film).
// An opaque black #afm-curtain is the first thing in <body>; the loader takes it down on every path (seen → the page; the film →
// dropped under the film's own black overlay; failure → the page). film-curtain.js runs in <head> before the first paint: it hides the
// curtain on a device that already watched the film to the end (unless this is a mail link or ?film), and otherwise starts fetching
// the bundle right away, next to the page's first read. CSS takes the curtain down by itself after 10 s, so voting is never blocked.
const CURTAIN=FILM_MODE==='first-visit';
if(CURTAIN&&!filmLoaderSrc.includes('afm-curtain'))fail('film-loader.js: expected the curtain hand-off (#afm-curtain)');
const CURTAIN_CSS='#afm-curtain{position:fixed;inset:0;z-index:2147482999;background:#000;animation:afm-curtain-safe .5s ease 10s forwards}'+
 '#afm-curtain::after{content:"";position:absolute;left:50%;top:50%;width:8px;height:8px;margin:-4px 0 0 -4px;border-radius:50%;background:#efe6d2;opacity:0;animation:afm-curtain-dot 1.4s ease-in-out 1.2s infinite}'+
 '#afm-curtain.up{opacity:0;transition:opacity .3s}html.afm-film-seen #afm-curtain{display:none}'+
 '@media (prefers-reduced-motion:reduce){#afm-curtain{display:none}}'+
 '@keyframes afm-curtain-safe{to{opacity:0;visibility:hidden}}@keyframes afm-curtain-dot{50%{opacity:.5}}';
const filmCurtain='// GENERATED by scripts/build-wanneer.mjs: the intro film\'s curtain, decided before the first paint (see CURTAIN there).\n'+
...
const page='<!doctype html><html lang="nl"><head><script src="/filmmaand/identity/reset-guard.js"></script>'+(CURTAIN?'<script src="/filmmaand/wanneer/film-curtain.js"></script>':'')+
 '<meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex">'+
 '<meta name="referrer" content="no-referrer"><title>Filmmaand</title>'+
 // the site's own icon, same as /stemmen/ and /identity/ (Chris, 23 Sept: the page had none)
 '<link rel="icon" href="/filmmaand/site/icons/interval-black.ico" sizes="any"><link rel="icon" href="/filmmaand/site/icons/interval-favicon.svg" type="image/svg+xml"><link rel="apple-touch-icon" href="/filmmaand/site/icons/interval-black-180.png">'+head+
 '<style>'+css+'</style><style>'+eggsCss+'</style><style>'+gamesCss+'</style><style>'+PAGE_CSS+'</style>'+(CURTAIN?'<style>'+CURTAIN_CSS+'</style>':'')+'</head><body>'+(CURTAIN?'<div id="afm-curtain" aria-hidden="true"></div>':'')+markup+
...
if(CURTAIN)await writeFile(root+'public/filmmaand/wanneer/film-curtain.js',filmCurtain);else await rm(root+'public/filmmaand/wanneer/film-curtain.js',{force:true});
