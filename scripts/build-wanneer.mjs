...
//     placeholder (filled in at runtime), and one empty system chip (#note) is added for connection messages;
...
const head='<!-- GENERATED by scripts/build-wanneer.mjs from the invitation kit (appje2.html sha256:'+sha(built).slice(0,16)+
 ', eggs.css sha256:'+sha(eggsCss).slice(0,16)+'). Do not edit: change the kit, then rebuild. -->';
const page='<!doctype html><html lang="nl"><head><script src="/filmmaand/identity/reset-guard.js"></script>'+
 ...
 '<style>'+css+'</style><style>'+eggsCss+'</style></head><body>'+markup+
 '<script src="/filmmaand/identity/avatars.js"></script><script src="/filmmaand/wanneer/wanneer.js"></script></body></html>\n';
...
// 7. Nothing from the fixture may ship.
const shipped={'index.html':page,'eggs.js':eggsOut,'wanneer.js':wanneer};
...
console.log(`wanneer: index.html ${(page.length/1024).toFixed(1)}K · eggs.js ${(eggsOut.length/1024).toFixed(1)}K · ${catName} ${catBytes.length} B · fixture names checked: ${fixtureNames.join(', ')}`);
