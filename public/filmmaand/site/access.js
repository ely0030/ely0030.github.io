...
async function ready(){
 ...
 const login=el('button','','Inloggen');login.type='button';login.onclick=()=>window.dispatchEvent(new CustomEvent('filmmaand-login-required',{detail:{source:kind}}));const back=el('a','','Bekijk het programma');back.href=prefix+'/programma/';prompt.append(heading,login,back);prompt.addEventListener('keydown',e=>{if(e.key!=='Tab')return;e.preventDefault();const nodes=[login,back],i=nodes.indexOf(document.activeElement);nodes[(i+(e.shiftKey?-1:1)+nodes.length)%nodes.length].focus({preventScroll:true});});prompt.addEventListener('cancel',e=>{e.preventDefault();location.assign(back.href)});document.body.append(prompt);prompt.showModal();
