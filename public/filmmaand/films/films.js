...
(() => {
  ...
  window.mountFilmsRanking=(host,ctx,actions)=>{
    ...
    const current=new Set(FilmsSocialModel.eligibility(ctx.plan).suspendedIds);
  ...
  function wireFriendRail(ctx){
    const rail=ctx.host.querySelector('.fs-new .fs-mini-rail');if(!rail||rail.dataset.dragBrowse)return;rail.dataset.dragBrowse='true';let drag=null,suppressUntil=0;
    rail.addEventListener('pointerdown',event=>{if(event.pointerType!=='mouse'||event.button!==0||event.target.closest('.hp-heart,.films-social-avatar,.hp-recommender,.fs-mini-author'))return;drag={id:event.pointerId,x:event.clientX,y:event.clientY,scroll:rail.scrollLeft,moved:false};});
    rail.addEventListener('pointermove',event=>{if(!drag||event.pointerId!==drag.id)return;const dx=event.clientX-drag.x,dy=event.clientY-drag.y;if(!drag.moved){if(Math.abs(dx)<6)return;if(Math.abs(dy)>Math.abs(dx)){drag=null;return;}drag.moved=true;rail.setPointerCapture(event.pointerId);rail.classList.add('is-browsing');}event.preventDefault();rail.scrollLeft=drag.scroll-dx;});
    const finish=()=>{if(!drag)return;const prior=drag;drag=null;if(prior.moved)suppressUntil=performance.now()+350;rail.classList.remove('is-browsing');if(rail.hasPointerCapture(prior.id))rail.releasePointerCapture(prior.id);};
    rail.addEventListener('pointerup',finish);rail.addEventListener('pointercancel',finish);rail.addEventListener('lostpointercapture',finish);rail.addEventListener('pointerleave',()=>{if(drag&&!drag.moved)drag=null;});
    rail.addEventListener('dragstart',event=>event.preventDefault());rail.addEventListener('click',event=>{if(performance.now()<suppressUntil){event.preventDefault();event.stopImmediatePropagation();}},true);
  }
